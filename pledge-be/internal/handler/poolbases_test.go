...
func newPoolbasesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPoolbasesDao(d.DB, c.ICache.(cache.PoolbasesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &poolbasesHandler{iDao: d.IDao.(dao.PoolbasesDao)}
	iHandler := h.IHandler.(PoolbasesHandler)
...
func Test_poolbasesHandler_Create(t *testing.T) {
	h := newPoolbasesHandler()
	defer h.Close()
	testData := &types.CreatePoolbasesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Poolbases))
...
func Test_poolbasesHandler_DeleteByID(t *testing.T) {
	h := newPoolbasesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Poolbases)
...
func Test_poolbasesHandler_UpdateByID(t *testing.T) {
	h := newPoolbasesHandler()
	defer h.Close()
	testData := &types.UpdatePoolbasesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Poolbases))
...
func Test_poolbasesHandler_GetByID(t *testing.T) {
	h := newPoolbasesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Poolbases)
...
func Test_poolbasesHandler_List(t *testing.T) {
	h := newPoolbasesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Poolbases)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPoolbasessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPoolbasesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPoolbasesHandler()
}
