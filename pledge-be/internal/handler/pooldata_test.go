...
func newPooldataHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPooldataDao(d.DB, c.ICache.(cache.PooldataCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &pooldataHandler{iDao: d.IDao.(dao.PooldataDao)}
	iHandler := h.IHandler.(PooldataHandler)
...
func Test_pooldataHandler_Create(t *testing.T) {
	h := newPooldataHandler()
	defer h.Close()
	testData := &types.CreatePooldataRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Pooldata))
...
func Test_pooldataHandler_DeleteByID(t *testing.T) {
	h := newPooldataHandler()
	defer h.Close()
	testData := h.TestData.(*model.Pooldata)
...
func Test_pooldataHandler_UpdateByID(t *testing.T) {
	h := newPooldataHandler()
	defer h.Close()
	testData := &types.UpdatePooldataByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Pooldata))
...
func Test_pooldataHandler_GetByID(t *testing.T) {
	h := newPooldataHandler()
	defer h.Close()
	testData := h.TestData.(*model.Pooldata)
...
func Test_pooldataHandler_List(t *testing.T) {
	h := newPooldataHandler()
	defer h.Close()
	testData := h.TestData.(*model.Pooldata)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPooldatasRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPooldataHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPooldataHandler()
}
