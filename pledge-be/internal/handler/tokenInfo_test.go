...
func newTokenInfoHandler() *gotest.Handler {
	...
	d.IDao = dao.NewTokenInfoDao(d.DB, c.ICache.(cache.TokenInfoCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &tokenInfoHandler{iDao: d.IDao.(dao.TokenInfoDao)}
	iHandler := h.IHandler.(TokenInfoHandler)
...
func Test_tokenInfoHandler_Create(t *testing.T) {
	h := newTokenInfoHandler()
	defer h.Close()
	testData := &types.CreateTokenInfoRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.TokenInfo))
...
func Test_tokenInfoHandler_DeleteByID(t *testing.T) {
	h := newTokenInfoHandler()
	defer h.Close()
	testData := h.TestData.(*model.TokenInfo)
...
func Test_tokenInfoHandler_UpdateByID(t *testing.T) {
	h := newTokenInfoHandler()
	defer h.Close()
	testData := &types.UpdateTokenInfoByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.TokenInfo))
...
func Test_tokenInfoHandler_GetByID(t *testing.T) {
	h := newTokenInfoHandler()
	defer h.Close()
	testData := h.TestData.(*model.TokenInfo)
...
func Test_tokenInfoHandler_List(t *testing.T) {
	h := newTokenInfoHandler()
	defer h.Close()
	testData := h.TestData.(*model.TokenInfo)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListTokenInfosRequest{query.Params{
		Page:  0,
		Limit: 10,
